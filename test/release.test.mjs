...
test('release identity and CLI guidance match bridge v0.1.1 with contract v0.1.0', async () => {
  const pkg = JSON.parse(await readFile(new URL('../package.json', import.meta.url)));
  const compatibility = JSON.parse(await readFile(new URL('../compatibility.json', import.meta.url)));
  assert.equal(BRIDGE_VERSION, '0.1.1'); assert.equal(pkg.version, BRIDGE_VERSION);
  ...
  assert.match(help.stderr, /v0\.1\.1/); assert.match(help.stderr, /editing\/local save and builds/);
