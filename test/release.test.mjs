...
test('release identity and CLI guidance match bridge v0.1.2 with contract v0.1.1', async () => {
  const pkg = JSON.parse(await readFile(new URL('../package.json', import.meta.url)));
  const compatibility = JSON.parse(await readFile(new URL('../compatibility.json', import.meta.url)));
  assert.equal(BRIDGE_VERSION, '0.1.2'); assert.equal(pkg.version, BRIDGE_VERSION);
  ...
  assert.equal(compatibility.contractVersion, '0.1.1');
  assert.deepEqual(pkg.bundleDependencies, ['@web64/mcp-contract']);
  assert.deepEqual(pkg.files, ['src', 'setup.mjs', 'setup.cmd', 'setup.command', 'README.md', 'CHANGELOG.md', 'compatibility.json', 'vendor/web64-mcp-contract-0.1.1.tgz']);
  ...
  assert.match(help.stderr, /v0\.1\.2/); assert.match(help.stderr, /editing\/local save and builds/);
